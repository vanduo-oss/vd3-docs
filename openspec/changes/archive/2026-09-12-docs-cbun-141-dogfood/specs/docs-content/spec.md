...
(`1.7.2` latest) and `@vanduo-oss/vd3-cbun` (`1.4.1` latest); they MUST NOT
name `@vanduo-oss/core`, `@vanduo-oss/framework`, `@vanduo-oss/vue`, or any of
the four old ecosystem packages, and MUST NOT carry any `data-engine`
attribute. The header copy SHALL name the two vd3 packages. Per the
changelog-content policy the page tracks package releases only, never
docs-site content. The latest `@vanduo-oss/vd3-cbun` card SHALL summarize
the published 1.4.1 notes for hex-grid, draw, charts, and code-editor.
...
- **THEN** every entry corresponds to an actual member of the published `@vanduo-oss/vd3-cbun/draw` surface (`VdDraw` props/emits and exposed `undo`/`redo`/`toSVG`/`toPNG`/`getInstance`/`setReadonly`/`setSnap`/`addShape`/`fitView`), with no invented API
...
### Requirement: hex pages dogfood the public hex-grid API

`/canvas/hex` and the `/cbun` hex preview SHALL render `VdHexGrid` from
`@vanduo-oss/vd3-cbun/hex-grid` using only the published type surface
(`VdHexGridCore`, `HexCell`, `HexRenderStats`). They MUST NOT assign or call
private members (`_getThemeColors`, `_render`, or equivalent). Theme/primary
changes that the package does not observe as CSS-variable mutations SHALL
remount the component (for example a theme-store `:key`) so the constructor
re-reads `--vd-*` tokens. Demo fills, terrain, and overlays SHALL use
`fillRandom`, `setHexFill`, `generateRandomTerrain`, and `setCustomRender`.

#### Scenario: hex demos stay on the public surface

- **GIVEN** `src/pages/canvas/Hex.vue` and `src/components/cbun/CbunPreviewHex.vue`
- **WHEN** they are grepped for `_getThemeColors` or `_render`
- **THEN** there are zero matches, and both files import hex types from
  `@vanduo-oss/vd3-cbun/hex-grid`
